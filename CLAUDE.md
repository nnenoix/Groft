...
- `core/` — только библиотеки: `context_store`, `decision_log`, `handoff`, `paths`, `logging_setup`, `subagent_ingest`, `step_planner`, `auto_learn`.
...
- ✅ **PR #4 (this):** auto-learn — `save_feedback_rule` MCP-тул, корректировки юзера пишутся в auto-memory одним вызовом.
- **Дальше:** Фаза 16 (Memory v2 — структура + ротация) — теперь когда все 4 write-source заведены, можно наводить порядок.
