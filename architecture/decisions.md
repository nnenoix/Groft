...
## 2026-04-20 — Phase 11: UX polish трёх dogfood-багов коммуникации

**Что:** три независимых фикса, каждый отдельным PR в ветке `feature/phase11.X-…`, в порядке **11.3 → 11.2 → 11.1** (от наиболее болезненного UX-блокера к самому масштабному refactor'у).

**11.3 — tmux submit sequence (PR #21, `329ff96`).** `TmuxBackend.send_text` теперь заканчивается на `C-j` (LF, 0x0A), а не на bare `Enter`. Причина — claude TUI (prompt-toolkit) интерпретирует `Enter` как newline в multi-line режиме, а `C-j` — как submit. Эмпирически подтверждено backend-dev'ом в отдельном tmux-окне. Новый kwarg `submit: bool = True` сохраняет совместимость с legacy `press_enter`. +6 тестов.

**11.2 — no pane fallback to lead (PR #22, `f0345b5`).** `_resolve_pane_target(to, *, fallback_to_lead=False)` возвращает `None` когда target неизвестен, вместо тихого routing'а в pane opus'а. `_forward_to_pane` логирует `pane forward skipped: unknown target=<name>` на info. Причина — fallback превращал MCP `send_message(to=X)` для out-of-band spawned X в echo-каскад `[from=sender] content` в opus'овом input-buffer'е, который harness ловил как user-messages. +5 тестов.

**11.1 — role-aware WS registry (PR #23, `608ac5d`).** `_registry: dict[agent, dict[role, ws]]` вместо плоского `dict[agent, ws]`. Register-фрейм принимает `role` (default `"worker"` для backward-compat). `core/main.py` регистрирует opus с `role="native"`, `mcp_server.py` — с `role="mcp-bridge"`. `_route_direct` фан-аутит обычные сообщения во все роли, но slash-команды (`/spawn /despawn /restart /decide /runtest /rescan-handoff /compact`) только в `native`. Без этого native opus и MCP-мост конкурировали за слот "opus", slash-команды от UI иногда попадали на MCP-мост (без dispatcher'а) и терялись. +7 тестов.

**Почему в обратном порядке приоритета (11.3 первым):**
- 11.3 блокирует dogfood: без submit'а любая MCP `send_message` теряется в input-buffer'е получателя. Фикс 11.3 первым даёт честный канал для опуса→агенты после рестарта orch.
- 11.1 большой refactor — разумно класть его последним, когда фундамент уже стабильный.
- 11.2 простой и изолированный — слот посередине.

**Архитектурное решение (11.1):** role-aware registry вместо `(name, client_id)` или `agent:role`-компаундного имени. Причина — имя `opus` остаётся атомарным в routing API (клиент `send_message(to="opus")` не знает про роли и не должен), но внутри сервера фан-аут вести по ролям. Это проще, чем переделывать все `to=<agent>` в `to=<agent>@<role>` через весь публичный API и client'ов.

**Реализация:** backend-dev в живом tmux-окне, три последовательных цикла (каждый PR = один dispatch → commit → push → opus-review → merge). Логические commits внутри ветки объединены в squash-merge. Тестовый рост: 174 → 180 → 185 → 192 passed (+18 тестов).

**Dogfood-подтверждение работы на себе же:** первая боевая проверка 11.3 произошла при запуске 11.2 — до рестарта orch код не активен, но тестово можно было проверить `send_text` вручную. Полная end-to-end валидация всех трёх — после рестарта orch (user-side action).

**Обновления memory:** `~/.claude/projects/…/memory/project_groft_dogfood_bugs.md` помечен `Status: FIXED в Phase 11`, сохранён как исторический контекст паттерна «dogfood находит связанную группу багов коммуникации».
